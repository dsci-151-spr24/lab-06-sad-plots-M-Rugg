...
***There should be 55 rows in the dataset when each row only has one year.***
...
staff_long %>%
  ...
  geom_line() +
  labs(
    title = "Faculty Trend over Time (1975-2011)", x = "", y = ""
  ) +
...
***To show that the proportion of part-time faculty has gone up over time in relation to the other insturctional staff types, you could highlight in color only the part-time faculty line. All the other lines could be left grey. This calls attention to the changes in part-time staffing relative to the other groups. Keeping the legend, however, can still be helpful to see what other instructional staff groupings are considered.***
...
**Ways to improve:**
 - remove or replace the pie charts
 - show the complete legend
 - replace the chart on the left with bars or some other form that does not indicate an ordered list 
 - Add a title and labels


**Changes made:**
 - removed the pie charts since they are not a good way of showing readable data
 - replaced the chart on the left with a bar chart since there is no valuable order to the countries
 - used a full, proportional bar chart to display the data to show the proportion of aquaculture to capture in each country
 - added legend, labels, title and subtitle
 - limited the data to 250,000 tons or more to increase readability of coutries
 - split the data into 2 separate bar charts (1,000,000+ tons and 250,000 - 500,000 tons) to further increase readability
 - adjusted the color for aesthetics
 - ordered the bars loosely by decreasing aquaculture siae (the graph uses proportions and the ordering used the minimum before proportions to reorder, however, I don't know how to do it elsewise)
...
fisheries %>%
  ...
  #arrange(capture/total) %>%
  select(-total) %>%
  pivot_longer(cols = -country, names_to = "type") %>%
  ggplot(aes(y = country, x = value, fill = type)) +
  geom_bar(stat = "identity", position = "fill") + scale_fill_manual(values = c("#9ecae1", "#3182bd")) + 
  labs(title = "Proportion of fishing types by country", subtitle = "1,000,000 tons or greater", x = "Proportion", y = "", fill = "") +
  #theme(legend.title = element_blank()) +
...
``` {r}
fisheries %>%
  filter(total >= 250000, total <= 1000000) %>%
  mutate(country = fct_reorder(country, aquaculture, min)) %>%
  #arrange(capture/total) %>%
  select(-total) %>%
  pivot_longer(cols = -country, names_to = "type") %>%
  ...
  #theme(legend.title = element_blank()) +
  theme(legend.position = "bottom") +
  labs(title = "Proportion of fishing types by country", subtitle = "250,000 - 500,000 tons", x = "Proportion", y = "", fill = "") +
  theme_minimal()
```
